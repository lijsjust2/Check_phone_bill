...
容器内访问运营商接口超时时（常见于 Docker 出口网络的 IPv6 / MTU 问题），登录面板后在浏览器打开：

```
http://面板地址:10086/api/diag/net
```

会输出各运营商接口 DNS / TCP / TLS / HTTP 四个阶段的计时与失败阶段，
据此可定位是 DNS 解析、TCP 连接、TLS 握手还是等服务端响应不通。
