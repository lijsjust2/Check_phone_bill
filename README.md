...
三网（移动 / 联通 / 电信）话费与套餐用量监控面板（Go 版）。单二进制 + Web 面板，支持多账号管理、定时查询、Bark/PushPlus 推送、2FA 登录保护。
...
- 中国联通：短信验证码登录（纯 HTTP 接口，无需浏览器与服务密码）
- 中国电信：服务密码登录（纯 HTTP 接口，token 长期有效，失效自动重登）
- 添加账号时自动按号段预选运营商，也可手动选择
- 账号列表：备注、手机号（脱敏显示，点击查看完整号码）、运营商、状态、余额、通用流量、语音、查询时间
- 单号查询 / 一键查询全部，登录态持久化，一次登录长期使用（联通 Cookie 失效自动会话维持刷新，电信 token 失效自动密码重登）
...
- 移动数据接口来自官网网页端（wx.10086.cn）；联通 / 电信接口来自官方 App 客户端协议，仅供个人学习使用
- 移动登录态与设备绑定，跨设备迁移请使用面板内置备份导出 / 导入；联通 / 电信为 HTTP 登录态，随备份一起迁移
- 避免频繁查询对运营商服务器造成压力
